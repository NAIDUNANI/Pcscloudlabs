				...
				<div class="row col-det">
					...
					<div class="col-lg-2 col-md-12">
						<div class="d-flex  justify-content gap-2" id="bk-demo">
							...
							<button class="btn-sec" onclick="showPopup('popup-demo')">
					...
					<!-- Single container that always occupies the same grid space -->
					<div class="col-lg-2 col-md-12" id="user-container">
						...
						<div class="login-signup"
							...
							<div class="d-flex justify-content gap-2">
								<button class="btn-sec" onclick="showPopup('popup-signup')">
									<i class="fas fa-calendar-alt"></i> Sign Up
								</button>
								<button class="btn-sec" id="btn-sec"
									onclick="showPopup('popup-login')">
									<i class="fas fa-sign-in-alt"></i> Login
								</button>
							</div>
							<!-- Popup overlay for Sign Up -->
							<div class="popup-overlay" id="popup-signup">
								<div class="popup-content ex_mar">
									<button class="close-btn" onclick="closePopup('popup-signup')">x</button>
									<img src="assets/images/logo.jpg" alt="PCS Cloud Lab Logo"
										style="width: 150px; margin-bottom: 10px;">
									...
									<form method="post" action="signup.jsp" id="signupForm">
										...
										<div style="display: flex; gap: 10px;">
											...
											</select> <input type="text" name="phone_number"
									...
									<p style="text-align: center;">
										Already have an account? <a href="#"
											onclick="showPopup('popup-login')"><b>Login</b></a>
									</p>
									<p style="text-align: center;">
							...
							<div class="popup-overlay" id="popup-login">
								<div class="popup-content">
									<button class="close-btn" onclick="closePopup('popup-login')">x</button>
									<img src="assets/images/logo.jpg" alt="PCS Cloud Lab Logo"
										style="width: 150px; margin-bottom: 10px;">
									...
									<p>
										Please Note: By continuing and signing in, you agree to PCS
										Cloud Lab's <a href="#">Terms & Conditions</a> and <a href="#">Privacy
											Policy</a>.
									</p>
									<p style="text-align: center;">
										Don't have an account? <a href="#"
											onclick="showPopup('popup-signup')"><b>Sign Up</b></a>
									</p>
								</div>
							</div>
						</div>
					...
					<div class="col-lg-9 col-md-12 nav-col">
						<nav class="navbar navbar-expand-lg navbar-light">
							...
							<div class="collapse navbar-collapse" id="navbarNav">
								<ul class="navbar-nav">
									...
									<li class="nav-item dropdown"><a
										...
										<div class="dropdown-menu" aria-labelledby="navbarDropdown">
											...
											</a> <a class="dropdown-item" href="salesforce_marketing.jsp">Salesforce Marketing
												Cloud</a> <a class="dropdown-item" href="#">Salesforce
									...
									<li class="nav-item dropdown"><a
										...
										<div class="dropdown-menu" aria-labelledby="navbarDropdown">
											<a class="dropdown-item" href="upcoming-events.jsp">Upcomining</a> <a
												class="dropdown-item" href="ongoing-events.jsp">OnGoing</a>
								...
								<a class="nav-btn" href="contact_us.html">
...
//Toggle dropdown menu when profile button is clicked
 ...
 document.addEventListener("click", function() {
  ...
  /* ======================================Scripting for the signup message Start here============================== */ 

  // Function to display signup message
/*   function displaySignupMessage(message) {
    const messageDiv = document.getElementById('signup-message');
    messageDiv.textContent = message;
    messageDiv.style.color = message.includes('Invalid') ? 'red' : 'green';
  }
  ...
  function validateSignup(event) {
    ...
    // Retrieve the values from the form (adjust the names to match your HTML)
    ...
    console.log('phone_number', phone_number);   // Debugging log
    console.log('Password:', password);     // Debugging log

    // Construct the request body
    const body = JSON.stringify({ email, phone_number, password });       
    console.log(form);
    ...
    fetch(form.action, {
      ...
      body: body, // Pass the email, country code, phone number, and password in the request body
    })
    .then(response => response.json()) // Assuming the server returns JSON with a message and success flag
    .then(data => {
      const signupMessage = data.message; // Get the message from the server's response
      console.log(signupMessage);
      displaySignupMessage(signupMessage);

      // If signup is successful, redirect after a delay
      if (data.success) { // Assuming the server sends a 'success' flag
        setTimeout(function () {
          window.location.href = 'index.jsp'; // Redirect to the home page (for example)
        }, 2000);
      }
    })
    .catch(error => {
  ...
  /* ======================================Scripting for the signup message Ends here============================== */

  /* ======================================Scripting for the login message Start here============================== */ 
...
function validateLogin(event) {
    ...
    fetch(form.action, {
        ...
        body: body, // Pass the email and password in the request body
    })
    .then(response => response.json()) // Assuming the server returns JSON with a message and success flag
    .then(data => {
        const loginMessage = data.message;
        displayLoginMessage(loginMessage); // Display the login message to the user

        // If login is successful
        if (data.success) { 
            // Hide the signup/login buttons
            const signupLogin = document.getElementById('signup-login');
            const profileSection = document.getElementById('profile-section');

            if (signupLogin && profileSection) {
                signupLogin.style.display = 'none';  
                profileSection.style.display = 'block'; 

                // Store login state in sessionStorage
                sessionStorage.setItem('isLoggedIn', 'true');
                sessionStorage.setItem('userEmail', data.email || 'Your Name');
            } else {
    ...
    .catch(error => {
        console.error('Error during login:', error); // Catch and log errors
        displayLoginMessage('An error occurred. Please try again.'); // Display error message to the user
    });
}
...
function logout() {
    ...
    // Show the login and signup buttons again
    const signupLogin = document.getElementById('signup-login');
    const profileSection = document.getElementById('profile-section');

    if (signupLogin) signupLogin.style.display = 'block';  // Show login/signup buttons
    if (profileSection) profileSection.style.display = 'none';  // Hide profile section

    // Redirect to the login page or home page
    window.location.href = 'index.jsp';  // or wherever you want to redirect after logout
}
...
window.onload = function() {
    ...
    const logoutLink = document.querySelector('a[href="index.jsp"]'); // Assuming this is your logout link

    if (isLoggedIn === 'true') {
        // Hide signup/login buttons
        if (signupLogin) signupLogin.style.display = 'none';  
        // Show the profile section
        if (profileSection) profileSection.style.display = 'block';  
        // Set the user's email in the profile section
        if (userEmailSpan) userEmailSpan.textContent = sessionStorage.getItem('userEmail') || 'Your Name';
    }

    // Attach the logout function to the logout link
    if (logoutLink) {
        logoutLink.addEventListener('click', function(event) {
            event.preventDefault(); // Prevent the default action (navigating to index.jsp)
  ...
  /* ======================================Scripting for the login message Ends here============================== */
